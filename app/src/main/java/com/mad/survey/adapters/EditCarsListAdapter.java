...
public class EditCarsListAdapter extends BaseAdapter{
	private Context ctx;
	private LayoutInflater inflater;
	private List<CarData> dataList;
    private OnClickListener mClickListener;

	public EditCarsListAdapter(Context ctx, LayoutInflater inflater, OnClickListener onClickListener){
		this.ctx = ctx;
	...
	public List<CarData> getListData(){
		return this.dataList;
	}

	public void setData(List<CarData> dataList){
		this.dataList.clear();
		this.dataList.addAll(dataList);
		notifyDataSetChanged();
	}

	public void appendData(Collection<CarData> dataList) {
		this.dataList.addAll(dataList);
		notifyDataSetChanged();
	}

	public void insertData(CarData note, int index) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final CarData data = dataList.get(position);

		holder.txtDescriptor.setText(ctx.getResources().getString(R.string.edit_car_item_title, data.getBankDesc(),(position+1),data.getCarNumber()));
